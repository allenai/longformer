...
class DistributedExecutor(object):
  ...
  # XLA_EMIT_STEPLOG emits a line to stderr every time mark_step is run. It is used for heartbeating.
  # We shortcircuit this logic.
  DIST_ENV_VARS = [
      'XRT_TPU_CONFIG',
      'XRT_LOCAL_WORKER',
  ...
  def _stream_logs(self, process, client_worker):
    ...
    def _stream_output(stream, log_fn):
      p = select.poll()
      p.register(stream, select.POLLIN)
      while True:
        if p.poll(0):  # no timeout
          line = stream.readline().decode('utf-8')
          if line == '':
            break  # subprocess terminated
          if 'torch_xla.core.xla_model::mark_step' in line:
  ...
  def _start_run(self, script_map):
    ...
    def _regular_health_check():
      ...
      #                                   1800)
      uneven_health_timeout = 1000000000
      even_health_timeout = 1000000000
      while True:
