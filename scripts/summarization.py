...
from longformer import LongformerT5ForConditionalGeneration, LongformerT5Config
...
class SummarizationDataset(Dataset):
    def __init__(self, hf_dataset, tokenizer, max_input_len, max_output_len, task_prefix=""):
    ...
    def __getitem__(self, idx):
        ...
        input_ids = self.tokenizer.encode(
            self.task_prefix + entry["article"], truncation=True, max_length=self.max_input_len
        )
...
class Summarizer(pl.LightningModule):
    def __init__(self, params):
        ...
        if "long" in self.args.model_path:
            if "bart" in self.args.model_path:
                ...
                model_class, config_class = LongformerT5ForConditionalGeneration, LongformerT5Config
            config = config_class.from_pretrained(self.args.model_path)
            ...
            self.model = model_class.from_pretrained(self.args.model_path, config=config)
        else:
    ...
    def _prepare_input(self, input_ids):
        ...
        if isinstance(
            self.model, (LongformerEncoderDecoderForConditionalGeneration, LongformerT5ForConditionalGeneration)
        ):
            attention_mask[
                :, 0
            ] = 2  # global attention on one token for all model params to be used, which is important for gradient checkpointing to work
    ...
    def _get_dataloader(self, current_dataloader, split_name, is_train):
        ...
        dataset = SummarizationDataset(
            task_prefix=("Summarize: " if "t5" in self.args.model_path else ""),
            hf_dataset=self.hf_datasets[split_name],
            tokenizer=self.tokenizer,
            max_input_len=self.args.max_input_len,
            max_output_len=self.args.max_output_len,
        )
        sampler = (
            torch.utils.data.distributed.DistributedSampler(dataset, shuffle=is_train) if self.trainer.use_ddp else None
        )
        return DataLoader(
            dataset,
    ...
    def configure_ddp(self, model, device_ids):
        model = LightningDistributedDataParallel(model, device_ids=device_ids, find_unused_parameters=False)
        return model
